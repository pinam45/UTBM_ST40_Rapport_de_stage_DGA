	...
	\chapter{YaCo}\label{ch:yaco}
		...
		\section{Les événements IDA}
			\subsection{Fonctionnement}
				...
				\end{figure}
				\paragraph*{}
					Lorsque l'utilisateur sauvegarde, IDA applique les modifications a son \gls{IDB} et envoie un événement aux hooks enregistrés. YaCo utilise alors les adresses stockées pour récupérer toutes les informations sur les zones modifiées dans un modèle qui est ensuite exporté en fichiers \gls{XML} avec un exporter \gls{XML}.
	%%% Bibliographie
	\nocite{*}
