...
\begin{document}
	\chapter*{Remerciements}
		...
		\paragraph*{}
			Je remercie aussi \textbf{la direction de la DGA} pour m'avoir permis de rejoindre leur personnel durant ces quelques mois ainsi que \textbf{Laure Foissard} du Bureau de Gestion des Emplois et Compétences de la DGA et \textbf{Mireille Jacquot} du Service des Stages de l'UTBM pour la gestion de mon dossier.
	\tableofcontents{}
	\listoffigures{}
	\chaptertoc{Introduction}
		...
		\end{figure}
		\paragraph*{}
			Ce rapport concerne mon ST40 que j'ai réalisé à la \gls{DGA} Maîtrise de l'Information sur le site de Bruz, près de Rennes du 1er Aout 2017 au 26 Janvier 2018 sous la supervision de mon maitre de stage, Benoît Amiaux.
		\paragraph*{}
			La \gls{DGA} est une direction du Ministère des Armées qui a de nombreuses missions dont celle de préparer l'avenir des systèmes de défense français, en particulier sur le plan informatique avec son centre d'expertise et d'essais Maîtrise de l'Information. De ce fait, la \gls{DGA MI} est un acteur français important de la sécurité informatique. Ce dernier proposant un sujet intéressant qui correspondait à mes compétences, j'ai donc saisi l'opportunité d'y réaliser mon stage.
		\paragraph*{}
			Le sujet du stage implique de nombreux concepts bas niveau, ce qui m'intéresse particulièrement et m'a en partie poussé à choisir ce sujet, préférant travailler plus proche de la machine.  Le sujet est le développement et la fiabilisation d'un module de désassemblage \acrshort{MIPS} pour un débogueur développé en interne. Le développement du module est en C/C++ et implique l'assemblage et le désassemblage de code, la gestion des adresses\ldots. Ainsi le sujet a l'avantage d'allier des concepts très bas niveau avec un développement dans des langages plus haut niveau.
		\paragraph*{}
			Je commencerai par présenter le Ministère des armées, la \gls{DGA} et la \gls{DGA MI}, j'expliciterai ensuite le sujet et l'organisation du stage avant de détailler le déroulement de ce dernier. Par la suite je développerai le travail réalisé sur le sujet initial ainsi que sur le sujet additionnel, ajouté au cours du stage.
	\chapter{Présentation de l'entreprise}
		...
		\section{La Direction générale de l'Armement}
			...
			\subsection{Organisation}
				\paragraph*{}
					Depuis le 9 août 2017, Joël Barre est Délégué Général pour l'Armement (voir organigramme en \annexe{organigrammedga}), ce dernier, pour mener à bien les missions de la \gls{DGA}, s'appuie sur différents services (dont les attributions exactes sont détaillées dans le décret n°2009-1180\cite{DEFD0918712D}), organisés la plupart du temps sous forme matricielle:
				...
				\p{Services de soutien} % décret n°2009-1180\cite{DEFD0918712D}
					Le \gls{SMQ}, le \gls{COMM}, le \gls{SSDI}, la \gls{DRH}, la \gls{GArm}.
	...
	\chapter{Organisation du stage}
		\section{GenDbg}
			\subsection{Présentation}
				...
				\paragraph*{}
					Les modules obligatoires au fonctionnement de GenDbg sont le stub, le framework, le module ASM (module de désassemblage) et le \gls{GUI}\footnote{interface graphique}. Historiquement seul un \gls{GUI} en ``mode texte'' était disponible mais une seconde interface, graphique cette fois, et réalisée avec Qt fut développée plus tard. Une vue de Gendbg utilisant le GUI Qt ainsi que sa lecture est présente en \annexe{guiqtgendbg}, ainsi que la lecture de la vue Code en \annexe{guiqtgendbgvuecode}.
				\begin{figure}[H]
					\centering
				...
				\paragraph*{}
					...
					\begin{itemize}
						\item Sur l'ordinateur 1:\\
							Exécuter le stub correspondant à l'architecture de l'ordinateur avec en paramètre le programme à analyser ainsi que le canal de communication à utiliser. Le stub se chargera de lancer l'application.
			...
			\subsection{Fonctionnement}
				\p{Le framework}
					C'est la partie centrale de GenDbg, c'est une application Win32 qui prend en charge la communication inter-modules, implémente le jeu de commandes de base commun à l'ensemble des modules ainsi que le moteur de script historique. Le moteur de script historique est maintenant très peu utilisé puisque des bindings Python sont  disponibles et utilisables dans une console Python intégrée au \gls{GUI} Qt, permettant de scripter directement en Python dans GenDbg.
				\paragraph*{}
				...
				\p{Modularité et généricité}
					De par son architecture (représentée sur la figure \ref{fig:GenDbg_modules}) GenDbg déplace les spécificités hors du framework, les spécificités d'architecture et d'OS à déboguer se trouvent donc au niveau des stubs tandis que celles dues à la représentation (instructions, adresses\ldots) se trouvent au niveau des modules ASM. De même, toutes les fonctionnalités spécifiques supplémentaires se trouvent au niveau des modules additionnels. Cela permet d'avoir un framework générique qui n'a pas besoin d'être modifié pour étendre les fonctionnalités de GenDbg. Cela permet aussi d'avoir un développement très ciblé, pour développer un nouveau module, il est seulement nécessaire de connaître l'\gls{API} que GenDbg expose aux modules et l'\gls{API} que le module expose à GenDbg.
				\p{Implémentation}
			...
			\subsection{Gestion du projet}
				...
				\paragraph*{}
					L'intégralité du code (framework, modules et stubs) est géré avec le gestionnaire de version Git, mais, pour des raisons de sécurité, il n'y a pas de serveur avec un dépôt synchronisé, ce dernier est passé par support physique entre les développeurs.
				\paragraph*{}
			...
			\subsection{Travail a réaliser}
				\paragraph*{}
					Le but initial du stage était le développement et la fiabilisation d'un module de désassemblage basé sur la librairie Capstone pour l'architectures \acrshort{MIPS} ou PowerPC. Le développement nécessitant d'implémenter toutes les fonctionnalités d'un module ASM GenDbg dont l'assemblage et le désassemblage en réalisant l'interfaçage entre GenDbg et Capstone. La fiabilisation elle, nécessitant la réalisation et la validation de tests unitaires sur le module développé.
				\begin{figure}[H]
					\centering
		...
		\section{YaCo}
			\subsection{Présentation}
				...
				\paragraph*{}
					IDA permet donc de comprendre le fonctionnement d'un programme en analysant manuellement un binaire après une première analyse automatique. Un problème apparait lorsque l'on veut réaliser cela sur des grosses bases de code, cela devient compliqué, voire impossible, de réaliser une analyse seul en un temps raisonnable, or IDA ne permet pas de travailler à plusieurs sur la même base de code.
				\begin{figure}[H]
					\centering
			...
			\subsection{Fonctionnement}
				...
				\paragraph*{}
					\gls{SWIG} est un outil logiciel open source qui permet de connecter des logiciels ou bibliothèques logicielles écrites en C/C++ avec des langages de script tel que le Python en créant des bindings pour ces derniers.
				\paragraph*{}
		...
		\section{Déroulement du stage}
			\subsection{Conditions de travail}
				\p{Horaires}
					Mes horaires de travail pendant le stage étaient 8h30 -- 16h45, mais ces derniers n'étaient pas stricts et il m'était possible de rester plus tard pour compenser en cas de retard le matin ou simplement pour finir quelque chose avant de partir. La pause de midi était de 45 minutes et en général l'équipe se réunissait dans le couloir pour aller manger ensemble au restaurant d'entreprise.
				\p{Badge et zones}
			...
			\subsection{Planning}
				...
				\paragraph*{}
					Après avoir terminé le travail sur GenDbg et YaCo, j'ai préparé une présentation orale interne à la \gls{DGA MI} sur le contenu de mon stage qui a eu lieu le 23 Janvier 2018 et a duré une quarantaine de minutes. Les 3 jours restants après la présentation, j'ai travaillé sur des améliorations mineures de YaCo.
	\chapter{GenDbg}\label{ch:gendbg}
		...
		\section{Compilation}
			\subsection{Le système en place}
				...
				\paragraph*{}
					Le système de compilation et de tests de GenDbg utilise CMake pour ne dépendre d'aucune plateforme ou compilateur particulier, pour cela un fichier (\file{CMakeLists.txt}) définit plusieurs cibles pour CMake. Chaque cible correspond à un ou plusieurs fichiers de sortie (\acrshort{DLL}, exécutable\ldots). Les principales cibles sont:
					\begin{itemize}
			...
			\subsection{Ajout du module}
				\paragraph*{}
					L'ajout du module a été assez simple, l'ancien module \acrshort{MIPS} étant présent, la configuration de CMake pour sa compilation était, elle aussi, déjà présente. Il a suffi de supprimer ce dernier pour le remplacer par une implémentation, initialement vide de l'\gls{API} des modules ASM pour que le nouveau module compile.
				\paragraph*{}
		...
		\section{Fonctionnement}
			...
			\subsection{Utilisation de Capstone et Keystone}
				\paragraph*{}
					Les fonctions d'assemblage et de désassemblage font partie des fonctions de conversion des données, celles-ci sont les plus complexes à implémenter et représentent la majeure différence avec l'ancien module MIPS de par l'utilisation de Capstone et Keystone.
				...
				\end{figure}
				\paragraph*{}
					\jfunc{UnassembleBloc} est utilisée pour afficher les instructions dans la vue code du GUI (visible dans l'\annexe{guiqtgendbgvuecode}), cette fonction est donc souvent appelée et a un grand impact sur les performances. Capstone bien optimisé, cela profite donc au module qui pourra aussi profiter des prochaines améliorations de Capstone sans avoir à changer son code.
				\paragraph*{}
				...
				\end{figure}
				\paragraph*{}
					Le problème de l'utilisation des informations détaillées de Capstone est que le format de ces informations est spécifique à l'architecture. Elles sont contenues dans la structure \jclass{cs\_detail} (voir \annexe{structurescapstone}) qui est différenciée pour chaque architecture avec une union. Les informations relatives à l'architecture MIPS se trouvent donc dans la structure \jclass{cs\_mips} (voir \annexe{structurescapstone}). Cela empêche un traitement unifié des informations détaillées pour plusieurs architectures.
			\subsection{Tests unitaires}
				...
				\paragraph*{}
					Les tests ont été réalisés avec \gls{GTest} qui ne nécessite pas de modifier le code du programme testé, permettant de garder les tests séparés du module. Les tests se déroulent dans une ``fixture'', c'est un environnement qui permet d'exécuter des tests, il est possible de configurer des actions réalisées avant et après chaque test.
				\paragraph*{}
				...
				\paragraph*{}
					De plus, j'ai conçu les tests de façon à garder les données des tests séparées du traitement des tests. Pour cela toutes les données à tester sont placées dans des tableaux de données de test. Ces tableaux permettent d'étendre simplement un test en ajoutant des lignes au tableau des données de test sans avoir à modifier le fonctionnement du test en lui-même.
				\paragraph*{}
		...
		\section{Difficultés}
			\paragraph*{}
				Les difficultés rencontrées ont été purement techniques, je vais en détailler uniquement une à titre d'exemple du type de problèmes rencontrés lors du développement du module.
			\paragraph*{}
		...
		\section{Résultats}
			...
			\paragraph*{}
				Un autre avantage de l'utilisation de Capstone et Keystone est que le module profitera des améliorations apportées à ces derniers dans leurs futures mises à jour, que ce soit des améliorations de performance ou le support des futures versions de l'\gls{ISA} \gls{MIPS}.
			\paragraph*{}
	...
	\chapter{YaCo}\label{ch:yaco}
		\section{Compilation}
			...
			\paragraph*{}
				Le processus de compilation commence par exécuter \gls{SWIG} avec ses fichiers de configuration qui spécifient les headers contenants les fonctions et classes pour lesquelles il faut générer des bindings. \gls{SWIG} génère alors un fichier C++ et les bindings Python qui lui sont associés. Le fichier C++ généré par \gls{SWIG} sera ensuite compilé avec la partie C++ de YaCo pour générer une \gls{DLL}.
			\paragraph*{}
		...
		\section{Les événements IDA}
			...
			\subsection{Implémentation}
				...
				\paragraph*{}
					IDA permet d'écouter différents types d'évènements (voir l'enum \jclass{hook\_type\_t} dans l'\annexe{idaapi}), parmi ces évènements, les évènements IDP (module processeur) sont les plus généraux et regroupent souvent de nombreuses modifications dans un seul évènement, mais ils permettent de suivre certaines des modifications de la base de code. Tandis que les évènements \gls{IDB} sont eux plus précis et permettent de suivre presque l'intégralité des modifications.
				\paragraph*{}
					Alors que la version Python des hooks utilisait certains évènements IDP car les évènements \gls{IDB} était incomplets dans l'\gls{API} Python, la version C++ n'utilise elle que les évènements \gls{IDB} car ces derniers sont beaucoup plus complets dans l'\gls{API} C++. Cela a permis le support de nouveaux évènements ainsi qu'une grande simplification du code.
				\paragraph*{}
			...
			\subsection{Résultats}
				\p{Améliorations}
					Le fait de passer de l'\gls{API} Python à l'\gls{API} C++ plus complète a permis le support de nouveaux évènements, les plus notables étant tous ceux en rapport avec les segments tels que le changement de position, de taille, de nom, de flags ou même de couleur.
				\paragraph*{}
				...
				\paragraph*{}
					Afin de permettre une évolutivité simple des hooks, j'ai ajouté à ces derniers l'analyse et le log de tous les évènements \gls{IDB} de IDA afin que l'ajout du support de nouveaux évènements soit le plus simple possible.
				\paragraph*{}
		...
		\section{Le dépôt Git}
			\subsection{Fonctionnement}
				\p{Git}
					Git est un système de contrôle de versions distribué, gratuit, et open-source conçu pour gérer le code source des petits aux très grands projets rapidement et efficacement. Il est très utilisé pour la gestion de tous types de projets informatiques et sur des sites d'hébergement de code tel que GitHub, pour plus d'informations se référer au site git-scm\cite{GitSCM}. Un schéma représentant les déplacements de données des commandes essentielles est disponible en \annexe{gitcommands}.
				\p{Synchronisation}
				...
				\paragraph*{}
					...
					\begin{itemize}
						\item Pas de modifications distantes: rien ne se passe
				...
				\begin{figure}[H]
					\centering%
					\resizebox{0.6\textwidth}{!}{\import{figures/}{YaCo_repository_discard_and_pull.tex}}%
			...
			\subsection{Implémentation}
				\paragraph*{}
					L'implémentation C++ du repository diffère peu de la version Python, le repository est géré par la classe \jclass{Repository}, cette dernière utilise la classe \jclass{GitRepo} de YaGitLib, une librairie de YaCo. YaGitLib est un wrap C++ de la librairie libgit2 qui est une implémentation C portable très utilisée des méthodes de base de Git.
				\paragraph*{}
					Le repository construit le message de commit au fur et à mesure des modifications de l'utilisateur. Les hooks, à chaque évènement reçut, préviennent le repository de la modification qui a eu lieu, le repository ajoute alors une ligne au message de commit avec l'adresse de la modification et des informations sur cette dernière.
			\subsection{Améliorations}
				\paragraph*{}
					Le portage en C++ n'apporte que peu d'améliorations, principalement la clarification et la simplification du code. Il a aussi permis de supprimer le besoin de \gls{SWIG} comme intermédiaire entre le repository et YaGitLib.
				\paragraph*{}
					La principale amélioration dont le repository aurait besoin est de changer la manière de générer les messages de commit ainsi que la syntaxe de ces derniers. En effet, avec le système actuel, si par exemple une structure est modifiée puis supprimée, le message de commit contiendra une ligne pour la modification puis une pour la suppression alors que seule celle de la suppression est pertinente.
				\paragraph*{}
					De plus, les indications sur les modifications restent assez vagues, les hooks avant le portage se contentant d'enregistrer l'adresse sans identifier précisément la modification. Or comme abordé précédemment, lors du portage des hooks en C++, ont été ajoutés des logs complets de tout les évènements avec une identification précise des modifications, ces informations pourrait donc aussi être utilisées pour le message de commit.
				\paragraph*{}
					Une idée de nouvelle syntaxe pour les messages de commit plus standardisée et permettant de connaitre les modifications exactes effectuées m'a été exposée, cette syntaxe pouvant aussi servir pour les logs des évènements. J'ai commencé à réfléchir à un moyen de réaliser cette dernière mais n'ai pas eu le temps de l'implémenter avant la fin du stage.
		\section{Résultats}
			\paragraph*{}
				Le portage de YaCo en C++ est presque complet, toutes les parties importantes ont été portées en C++, ne reste en Python plus que le fichier \file{yaco\_plugin.py} qui sert à lancer le plugin, et les tests. Ces derniers vont sans doute rester en Python, yaco\_plugin ne servant qu'à ajouter YaCo comme plugin a IDA et les tests ne servant que pour le développement.
			\p{Avantages}
				Le plus visible est un énorme gain de performances mais cela vient presque exclusivement des modèles / exporteurs déjà portés en C++ avant mon arrivée. Le portage des hooks et du repository ont eu surtout pour avantage d'avoir accès a une \gls{API} IDA beaucoup plus complète, ce qui a permis d'ajouter le support de nouvelles modifications mais aussi permettra, dans le futur, un meilleur système de logs et messages de commits.
			\paragraph*{}
				Le portage a aussi pour avantage de passer d'un langage interprété à un langage compilé, simplifiant énormément le suivi des changements d'\gls{API} IDA, puisque au lieu de découvrir les modifications à l'exécution, ces dernières sont directement détectées à la compilation. Par exemple au passage à IDA 7, certaines fonctions ont vu l'ordre de leurs paramètres changer, pour la partie C++ une simple compilation a permis de détecter cette modification puis de s'assurer d'avoir corrigé toutes les utilisations de ces fonctions.
			\paragraph*{}
				En Python la modification est invisible, le plugin fonctionne toujours et une erreur ne sera reportée que lors de l'utilisation de la fonction qui a changée. Il faut aller chercher les utilisations de cette dernière à la main, quand la modification est bien précisée dans le changelog d'IDA, ce qui n'est pas toujours le cas.
			\paragraph*{}
				Le C++ apporte aussi un débogage plus simple, permettant de suivre le programme à son exécution et d'utiliser une version Debug, tandis que en Python ce dernier est exécuté par IDA en Release empêchant le débogue. De plus, le débogue C++ est beaucoup plus puissant que celui du Python de par le fait que le langage soit explicitement fortement typé mais aussi que les outils disponibles sont beaucoup plus développés (par exemple de débogueur de Visual Studio).
			\paragraph*{}
				Finalement, le portage a permis de repenser le code de façon globale afin de le simplifier, ajouter de nouvelles fonctionnalités, supprimer ce qui ne servait plus, et de simplifier les évolutions futures.
			\p{Inconvénients}
				Les principaux utilisateurs de YaCo ne sont pas des développeurs à plein temps, leur principale occupation est le reverse engineering, c'est pour cela qu'ils utilisent IDA et donc YaCo. Ces derniers se servent donc plus souvent de langages de script tels que le Python (IDA est équipé d'une console Python) que de langages tels que le C++. Le portage rend donc le développement de YaCo moins accessible à ses principaux utilisateurs.
			\paragraph*{}
				On peut aussi noter que le C++ nécessite plus de code par rapport au Python pour réaliser une tâche équivalente, de ce fait, bien que le code ait été simplifié avec le portage, ce dernier a augmenté la quantité de code de YaCo.
			\paragraph*{}
				De mon point de vue, les avantages de ce portage surpassent largement les inconvénients qui l'accompagnent, notamment pour l'évolution du plugin qui sera beaucoup plus simple avec une compilation et un débogue facile à réaliser.
	\chaptertoc{Conclusion}
		\p{Entreprise}
			La \gls{DGA} est vraiment une entreprise particulière de par son fonctionnement et l'importance du secret, ce qui a été très intéressant à découvrir et m'a donné envie de m'intéresser à la sécurité dans la suite de mes études. J'ai découvert à la \gls{DGA MI} des métiers de l'informatique qui m'étaient inconnus, notamment ceux liés au reverse engineering que je trouve très intéressants et peu enseignés, même si je garde une préférence pour le développement pur.
		\paragraph*{}
			La \gls{DGA MI} a l'avantage par rapport aux autres entreprises ``standard'' de ne pas avoir un but de rentabilité, cela lui permettant de regrouper des experts dans tous les domaines de l'informatique sur des sujets variés. J'avais déjà vu des communications du Ministère des armées mais, avant ce stage, je n'avais pourtant jamais entendu parler de la \gls{DGA MI} qui semblait très fermée. Cependant cette dernière commence à s'ouvrir, cela est visible notamment par les présentations réalisées au \gls{SSTIC} mais aussi par les projets publiés publiquement dont YaCo est le premier.
		\p{Travail produit}
			Je suis content de ce que j'ai réalisé durant le stage, le sujet initial étant la réalisation du module MIPS, ce dernier est parfaitement fonctionnel et équipé de tests comme voulu initialement. De plus, ayant fini le module rapidement, j'ai eu l'occasion de travailler sur YaCo, le repository et les hooks que j'ai portés en C++ sont fonctionnels et apportent des améliorations par rapport à leur anciennes versions.
		\paragraph*{}
			Des évolutions de ces derniers sont encore possibles, notamment le support de coprocesseurs pour le module \gls{MIPS} de GenDbg et les messages de commits et de logs de YaCo.
		\p{Expérience}
			Je me suis intéressé à beaucoup de sujets que j'ai trouvés passionnants pour lesquels je n'aurais peut-être pas cherché d'informations sans ce stage tels que l'architecture des microprocesseurs, le rétro-engineering et l'analyse de binaires. Le travail que j'ai réalisé ainsi que les discussions avec les membres de la division et surtout avec mon maitre de stage m'ont permis de comprendre les contraintes réelles du développement d'un logiciel. Par exemple l'importance de la conception initiale, GenDbg après plus de 10 ans reste limité par les choix d'\gls{API} faits initialement et YaCo, pour rester rétro-compatible, ne peut pas se permettre de changer la syntaxe \gls{XML} lourde choisit initialement.
		\paragraph*{}
			Ce stage a été l'occasion de travailler sur des projets massifs avec lesquels j'ai pu me confronter à de nombreux aspects de conception, notamment me rendre compte de l'importance de penser aux données et à leur représentation et pas uniquement à leur traitement. Dans le cas d'un projet scolaire, on a tendance à oublier ce point, ce dernier brassant peut de données cela n'a que peu d'impact sur les performances. Mais, lorsque les données commencent à se compter en gigaoctets, avoir une approche ``data oriented'' est très important, notamment au niveau des \gls{API} qui, si mal conçues, peuvent forcer une représentation des données à l'implémentation. L'approche ``object oriented'' seule étant souvent inefficace pour un projet brassant une grande quantité de données.
	%%% Bibliographie
	\nocite{*}
	...
	\chaptertoc{Annexes}\markboth{ANNEXES}{}
		...
		\section{Organigramme de la DGA}\label{sec:organigrammedga}
			\begin{figure}[H]
				...
				\caption{Organigramme de la \acrshort{DGA}\cite{OrganigrammeDGA}}
		...
		\newpage\section{Hiérarchie des Ministères Français à VIM/VSE}\label{sec:hierarchieministereavim}
			\begin{figure}[H]
				...
				\caption{Hiérarchie des Ministères Français à \acrshort{VIM/VSE}}
		...
		\newpage\section{Extraits de l'API IDA}\label{sec:idaapi}
			\lstinputlisting[language={[yaco]C}, style=magic_style]{code/hook_to_notification_point_unhook_to_notification_point.c}
			\lstinputlisting[language={[yaco]C}, style=magic_style]{code/hook_type_t.c}
		\newpage\section{Git: commandes essentielles}\label{sec:gitcommands}
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.7\textwidth]{Git_commands}
				\caption{Git: commandes de déplacement de données\cite{GitCommands}}
