...
\begin{document}
	...
	\chapter{YaCo}\label{ch:yaco}
		...
		\section{Le dépôt Git}
			...
			\subsection{Implémentation}
				...
				\paragraph*{}
					Le repository construit le message de commit au fur et à mesure des modifications de l'utilisateur, à chaque évènement reçut par les hooks, ces derniers préviennent le repository de la modification qui a eu lieu et ce dernier ajoute une ligne au message de commit avec l'adresse de la modification et des informations sur cette dernière.
		...
		\section{Résultats}
			...
			\paragraph*{}
				Le portage a aussi pour avantage de passer d'un langage interprété à un langage compilé, simplifiant énormément le suivi des changements d'\gls{API} IDA, puisque au lieu de découvrir les modifications à l’exécution, ces dernières sont directement détectés à la compilation. Par exemple au passage à IDA 7, certaines fonctions ont vu l'ordre de leurs paramètres changer, pour la partie C++ une simple compilation a permis de détecter cette modification puis de s'assurer d'avoir corrigé toutes les utilisations de ces fonctions.
			\paragraph*{}
	...
	\chaptertoc{Conclusion}
		...
		\paragraph*{}
			La \gls{DGA MI} a l'avantage par rapport aux autres entreprises ``standard'' de ne pas avoir un but de rentabilité, cela lui permettant de regrouper des experts dans tous les domaines de l'informatique sur des sujets variés. J'avais déjà vu de la communication du Ministère des armées mais, avant ce stage, je n'avais pourtant jamais entendu parler de la \gls{DGA MI} qui semblait très fermée. Cependant cette dernière commence a s'ouvrir, cela est visible notamment par les présentations réalisées au \gls{SSTIC} mais aussi par les projets publiés publiquement dont YaCo est le premier.
		\p{Travail produit}
