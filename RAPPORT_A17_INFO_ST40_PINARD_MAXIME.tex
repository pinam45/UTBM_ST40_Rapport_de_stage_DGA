			...
			\subsection{GenDbg}
				\subsubsection{Présentation}
					...
					\paragraph*{}
						...
						\end{itemize}
					\paragraph*{}
						GenDbg permet de déboguer tout type de programme (code machine, byte code java, P-Code visual basic, DotNet,\ldots) sur des architectures hétérogènes. Pour cela, il adopte un modèle \gen{Framework} / \gen{Stub} qui permet de séparer les composants débogués et la machine de l'analyste. En fonction du type d'analyse envisagé (malware) on préférera travailler sur deux machines physiques distinctes. Cette approche avec des \gen{Stubs} permet aussi de réduire l'empreinte du débogueur sur la cible. Le \gen{Framework} est capable de communiquer avec les \gen{Stubs} via un port série ou des canaux nommés (pipes).
					\paragraph*{}
						Les modules obligatoires au fonctionnement de GenDbg sont le \gen{Stub}, le \gen{Framework}, le \gen{Module ASM} (module de désassemblage) et le \gen{\gls{GUI}}\footnote{interface graphique}. Historiquement seul un \gen{\gls{GUI}} en ``mode texte'' était disponible mais une seconde interface, graphique cette fois, et réalisée avec Qt fut développé plus tard. Une vue de Gendbg utilisant le GUI Qt ainsi que sa lecture est présente en \annexe{guiqtgendbg} ainsi que la lecture de la vue Code en \annexe{guiqtgendbgvuecode}.
					\begin{figure}[H]
						\centering
					...
					\paragraph*{}
						...
						\begin{itemize}
							\item Sur l'ordinateur 1:\\
								Exécuter le \gen{Stub} correspondant à l'architecture de l'ordinateur avec en paramètre le programme a analyser ainsi que le canal de communication a utiliser. Le \gen{Stub} se chargera de lancer l'application.
							\item Sur l'ordinateur 2:\\
								Spécifier le canal de communication à utiliser dans le fichier de configuration de GenDbg puis exécuter le \gen{Framework}. Le \gen{Framework} se chargera des récupérer les informations nécessaires du \gen{Stub}, tel que le \gen{Module ASM} a utiliser et chargera le \gls{GUI}, permettant d'exécuter l'application pas-à-pas.
						\end{itemize}
					\paragraph*{}
