...
\begin{document}
	...
	\begin{tikzpicture}[x=1pt,y=1pt,>=Latex]
		...
		\draw[
			fill=color3,
		] (stub.east) -- ($(stub.east) + (stub.south) - (stub)$) -- (stub.south) -- cycle;
		\node[
			xshift=10,
			yshift=-3,
			font=\footnotesize,
		] (exe) at (stub.south east) {exe};
		...
		\draw[
			fill=color3,
		] (gendbg.west) -- ($(gendbg.west) + (gendbg.south) - (gendbg)$) -- (gendbg.south) -- cycle;
		\node[
			xshift=-11,
			yshift=-3.5,
			font=\footnotesize,
		] (exe) at (gendbg.south west) {exe};
