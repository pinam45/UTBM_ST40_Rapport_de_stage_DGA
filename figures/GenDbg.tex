...
\begin{document}
	\def\computerwidth{210}
	\def\computerheight{200}
	\def\xstartmodules{15}
	\def\ystartmodules{-10}
	\def\yshiftmodules{-5}
	\def\osiconsheight{25}
	...
	\begin{tikzpicture}
		...
		\node[anchor=south west] (win) at (c1.south west) {\includegraphics[height=\osiconsheight pt]{Windows.png}};
		\node[anchor=west] (tux) at (win.east) {\includegraphics[height=\osiconsheight pt]{Tux.png}};

		\node[comp] (c2) at (10,0) {};
		\node[anchor=south] (c2t) at (c2.north) {Ordinateur 2};
		\node[anchor=south west] (win) at (c2.south west) {\includegraphics[height=\osiconsheight pt]{Windows.png}};

		%---------------------------------
		% Stub
		\node[
			isosceles triangle,
			isosceles triangle apex angle=43,
			xshift=90,
			yshift=40,
			minimum width=110,
			fill=cyan,
