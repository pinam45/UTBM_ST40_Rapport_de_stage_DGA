...
\begin{document}
	\begin{tikzpicture}[x=1pt,y=1pt]
		...
		\begin{scope}[x={(image.south east)},y={(image.north west)}]
			...
			\draw[<-,>=latex] (currentline) -- ++(0.1,0) node[txt,anchor=west,text width=90] {instruction courante (saut conditionnel)};
			...
			\draw[<-,>=latex] (jump1) -| (jumps);
			\draw[<-,>=latex] (jump2) -| (jumps);
			\draw[<-,>=latex] (jump3) -| (jumps);
			\draw[<-,>=latex] (jump4) -| (jumps);
			\draw[<-,>=latex] (jump5) -| (jumps);
